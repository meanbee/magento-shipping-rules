...
export default {
    ...
    constructInputField: function (condition) {
        let comparator = Meanbee.ShippingRules.ajax.getComparators(condition.attribute).filter(x => x.value === condition.comparator)[0];
        let conditionField = Meanbee.ShippingRules.ajax.getConditionFieldByValue(condition.attribute);
