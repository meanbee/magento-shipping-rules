<?php
abstract class Comparator_Abstract
{
	private $types = array();
	...
	public function removeType($typeID, $bidi = true)
	{
		if ($this->canHandleType($typeID, $bidi)) {
			...
			if ($bidi) {
				$type = Register_Type::instance()->get($typeID);
				$comparatorID = Register_Comparator::instance()->find($this);
	...
	public function canHandleType($typeId, $bidi = true)
	{
		...
		if ($bidi) {
			$comparatorID = Register_Comparator::instance()->find($this);
			return Register_Type::instance()->get($typeId)->canBeHandledByComparator($comparatorID, false);
		}
		return false;
	}
}
